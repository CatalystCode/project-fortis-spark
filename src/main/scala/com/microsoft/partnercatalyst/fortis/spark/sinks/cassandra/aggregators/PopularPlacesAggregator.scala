...
import com.microsoft.partnercatalyst.fortis.spark.sinks.cassandra.dto.{EventBatchEntry, PopularPlaceAggregate}
import org.apache.spark.sql.{DataFrame, Dataset, SparkSession}

class PopularPlacesAggregator extends FortisAggregator with Serializable{
  ...
  private def DetailedAggregateViewQuery: String = {
    ...
    val SelectClause = GroupedColumns.mkString(",")
      ...
      s"FROM   $DataFrameNameFlattenedEvents " +
  ...
  private def IncrementalUpdateQuery: String = {
    ...
    s"SELECT a.$SelectClause, current_timestamp() as insertiontime, " +
    s"       $IncrementalUpdateMentionsUDF, $IncrementalUpdateSentimentUDF " +
    s"FROM   $DataFrameNameComputed a " +
  ...
  override val FortisTargetTablename: String = TargetTableName
  ...
  override def flatMap(session: SparkSession, eventDS: Dataset[EventBatchEntry]): DataFrame = {
